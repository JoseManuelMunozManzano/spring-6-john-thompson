...
public class SecurityConfig {
    ...
    // Aquí configuramos JWKSource, que sirve para firmar el token JWT con la key privada.
    // También indicamos la forma en que el authentication server va a exponer los endpoints para
    // proveer la key pública.
    //
    // Cuando el resource server obtiene el token JWT, puede volver a llamar al authorization server,
    // obtener la key pública y validar que el token JWT tiene un payload válido.
    // Así, el resource server sabe que puede confiar en el token JWT.
    @Bean
    ...
    // Responsable de decodificar JWT para poder validarlos.
    @Bean
    public JwtDecoder jwtDecoder(JWKSource<SecurityContext> jwkSource) {
        return OAuth2AuthorizationServerConfiguration.jwtDecoder(jwkSource);
    }

    // Una instancia de AuthorizationServerSettings para configurar Spring Authorization Server
